...
# <$1 : séparation / réutilisation des fichiers #
# <stdin : data+payload                         #
# >stdout : type de l'entrée                    #
...
lookup() {
  ...
  sfx="${1:-$RANDOM}"
  local result="$(tee --output-error=warn lookup."$sfx" | file -bi -)"
  ...
  if [[ $result =~ application/gzip ]];then
    ...
    if grep -aq '### BEGIN SEALGOOD' lookup."$sfx".unzip;then
      ...
      if ((count)); then
        ...
        zcat >lookup."$sfx".payload &&
        zcat lookup."$sfx".original >lookup."$sfx".unzip &&
        return 0
      fi <lookup."$sfx" 2>/dev/null
    fi
    cp -lf lookup."$sfx"{,.original}
    echo > lookup."$sfx".payload
