...
inject() {
  ...
  if ((rargs == 0));then
    ...
    if ! [[ $filetype =~ sealgood ]]; then
      ...
      get_payload <lookup.inject.original >lookup.inject.payload
      chmod -w lookup.inject.payload
    fi
    inject_type inject "$filetype" | tee >(echo "inject output: $(lookup)" >&2)
      ...
      if <lookup.date.payload grep -aq "^PLACEHOLDER_UNTIMESTAMPED_FILE" ||
        ! openssl ts -verify -in tsr.tmp -CAfile <(<lookup.date.payload tsa_cert) -data sig.tmp &> ts_result; then
          ...
          if [ -s sig.tmp ]; then
            ...
            ts_verify="openssl ts -verify -in /tmp/tsr.bin -CAfile /tmp/cacert.pem -data /tmp/sig.bin # $(_ "verify timestamp")"
          else
            cat lookup.date.original # Horodatage sur le document original (ancienne méthode)
            ts_verify="openssl ts -verify -in /tmp/tsr.bin -CAfile /tmp/cacert.pem -data /tmp/\$filename.pdf # $(_ "verify timestamp")"
          fi |
          timestamp | base64 -w 0 | tr -d ' \n' >tsr.64 ||
          exit_message="$(_ "Timestamp failed")"
      else
        exit_message="$(_ "Already timestamped")"
      fi
      if [ -s tsr.64 ];then
  ...
  if ((rargs == 0));then
    ...
    if [[ $filetype =~ sealgood ]];then
      ...
      if grep -aq "^PLACEHOLDER_UNSIGNED_FILE" lookup.sign.payload; then
        ...
        REXP="s|^PLACEHOLDER_UNSIGNED_FILE|$(signraw | base64 -w 0)|"
        if [[ $filetype =~ gzip ]];then
          cat lookup.sign.original
          <lookup.sign.payload sed -e "$REXP" | gzip -nc
        else
          <lookup.sign         sed -e "$REXP" # DANGER : lit tout le doc
        fi
      else
        cat lookup.sign
      ...
      if [ -s tsa_cert.pem ] || [ -s public_key.pem ]; then
        ...
        if [ -s tsr.bin ] && [ -s tsa_cert.pem ] && [ -s lookup.verify.original ] ; then
          ...
          if [ -s signature.bin ];then
            (
              # <lookup.verify.payload grep -qa \
              # -e 'openssl ts -verify -in /tmp/tsr.bin -CAfile /tmp/cacert.pem -data /tmp/sig' &&
              set -x
...
awk '/^${BTAG}/{state=1}!state{print}/^$ETAG/{state=0}' < "\$filename.pdf" > /tmp/\$filename.pdf # $(_ "Unsigned file without payload")
...
# $(_ "COPY the three lines below and PASTE into file") /tmp/cacert.pem
# $(_ "or download %s to %s" $tsa_ca /tmp/cacert.pem)
$(
  [ -s cacert.pem ] ||
  curl -s $tsa_ca |
...
openssl x509 -reply -text -in /tmp/cacert.pem # $(_ "show certificate details")
...
openssl ts -verify -in /tmp/tsr.bin -CAfile /tmp/cacert.pem -data /tmp/sig.bin # $(_ "verify timestamp")
openssl ts -verify -in /tmp/tsr.bin -CAfile /tmp/cacert.pem -data /tmp/\$filename.pdf # $(_ "verify timestamp")
...
tsa_cert() {
  if ! [ -s tsa_cert.pem ];then
    ...
    if ! [ -s tsa_cert.pem ]; then
      warning "$(_ "TSA certificate not found - Downloading from") $tsa_ca"
      curl -s $tsa_ca >tsa_cert.pem ||
...
timestamp() {
  ...
  curl -s -H "Content-Type: application/timestamp-query" --data-binary @- --output - $tsa_serv |
  perl -ne 'BEGIN { binmode STDIN; binmode STDOUT; $err = 0 }
    if (/error/i) {
      $clean = $_;
      $clean =~ s/[^[:print:]\t\n\r]//g;
      print STDERR "$tsa_serv: ".$clean."\n";
      $err++;
      next
    }
    print;
...
signraw() {
  if ((fdpassin)); then
    echo ask >&"$fdpassask"
    openssl pkeyutl -sign -inkey "$PRIVATE_KEY" -rawin -in lookup.sign.original -passin stdin <&"$fdpassin"
  else
    openssl pkeyutl -sign -inkey "$PRIVATE_KEY" -rawin -in lookup.sign.original
  fi </dev/null
}
