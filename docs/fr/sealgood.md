  ...
  if ((rargs == 0));then
    ...
    if [[ $filetype =~ sealgood ]];then
      if [[ $filetype =~ gzip ]];then
        local count=$((gzip -cd lookup.clean || die $LINENO assert) | awk '/### BEGIN SEALGOOD /{state=1}state && /^wc *: /{print $5;exit}')
        head -c "$count"
      else
    ...
    if [[ $filetype =~ sealgood ]];then
      ...
      if [ -s tsr.64 ];then
        ...
        if [[ $filetype =~ gzip ]];then
          ...
          <lookup.date.payload sed "s|$REXP|$(cat tsr.64)|" | gzip -nc
        else
    ...
    if [[ $filetype =~ sealgood ]];then
      if [[ $filetype =~ gzip ]];then
        if zgrep -aq "^PLACEHOLDER_UNSIGNED_FILE" lookup.sign; then
          ...
          local count=$((gzip -cd lookup.sign || die $LINENO assert) | awk '/### BEGIN SEALGOOD /{state=1}state && /^wc *: /{print $5;exit}')
          head -c "$count"
          (gzip -cd || dier $LINENO assert) | sed "s|^PLACEHOLDER_UNSIGNED_FILE|$(<lookup.sign signraw | base64 -w 0)|" | gzip -nc
        else
          cat
  ...
  if ((rargs == 0));then
    ...
    if [[ $filetype =~ sealgood ]];then
      ...
      if ! [ -s signature.bin ]; then
        ...
        <lookup.verify.payload awk '/-----BEGIN PUBLIC KEY-----/,/-----END PUBLIC KEY-----/' payload | head -n 3 >public_key.pem
      ...
      if [ -s tsa_cert.pem ] || [ -s public_key.pem ]; then
        ...
        if [ -s public_key.pem ] && [ -s signature.bin ] && [ -s original_data ]; then
          ...
          (
            set -x
            openssl pkeyutl -verify -pubin -inkey public_key.pem -sigfile signature.bin -in lookup.verify.original -rawin &> sig_result ||
            openssl pkeyutl -verify -pubin -inkey public_key.pem -sigfile signature.bin -in <(openssl dgst -sha256 -binary lookup.verify.original) &> sig_result | cat
          )
          SIG_VERIFY=$?
...
lookup() {
  ...
  if ! [[ $result =~ /x-empty ]];then
    ...
    if ! [ -f lookup."$sfx".payload ];then
      # Tentative de parsing du lookup."$sfx".payload selon le filetype
      ...
      [ -f lookup."$sfx".unzip ] || cp -l lookup."$sfx" lookup."$sfx".unzip
      ...
      if [ -s lookup."$sfx".payload ];then
        ...
        then
          ...
          (( count == $(wc -c <lookup."$sfx".original) )) ||
